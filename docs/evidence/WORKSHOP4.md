...
- Added the OWASP Dependency-Check Maven plugin to the parent `pom.xml`
  (`org.owasp:dependency-check-maven`, version `10.0.4`).
...
- The plugin runs explicitly (`mvn org.owasp:dependency-check-maven:check`) rather than being
...
- [ ] `mvn clean verify` passes (no functional regressions).
...
- [ ] Swagger is reachable only under the `dev` profile.
